...
- **`human-training:robustness-audit`** reads existing code across several surfaces looking for
...
The script exercises the real artifact and **fails loud if you are wrong**. A
script that prints something you then interpret is not proof; one that exits
non-zero when the assumption breaks is.

**"The real artifact" is not always a running program.** In an application it is
the same import the app uses, the same function, the same inputs. In a repo whose
product is configuration, documents, or generated output, it is the built tree
and the manifests that ship — assert against *those*. The test is whether the
script fails when the fact is false, not whether it launched something.

**A failing proof is the finding.** If the script goes red, you have not made a
mistake in the script by default — you have found the breakage, which is the
whole point. Report it, fix it, or say why it is acceptable. Re-running until it
goes green and reporting only the green run is the exact dishonesty this skill
exists to prevent.

Keep the script. A reviewer can rerun it, and it is the artifact that turns
"I checked" into "run this". If it caught something real, it wants to become a
committed check rather than a scratch file.
...
- **Running this as a general audit.** No change in view means the wrong skill —
  that's `human-training:robustness-audit`.
- **Reporting only the green run.** If the proof went red first, that is the
  finding and it belongs in the report, alongside what you did about it.

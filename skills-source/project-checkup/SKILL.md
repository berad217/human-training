...
- **A toolchain that has stopped updating.** Read
  `~/.claude/plugins/known_marketplaces.json` — one entry per marketplace, each
  carrying `autoUpdate` and `lastUpdated`. A marketplace whose `lastUpdated` is
  months old is running this project's tooling on a stale catalog, and
  `claude plugin update` will keep answering "you're already current" because it
  compares against that catalog. `autoUpdate` absent removes the guarantee of a
  refresh, though it does not strictly prevent one — so **report on the two
  fields, not on a theory of the mechanism**: a stale `lastUpdated` is the
  finding whether or not you can explain the stall. The fix is
  `claude plugin marketplace update <name>` *then* `claude plugin update
  <plugin>@<name>` — the first line is the one everyone skips, and without it the
  second consults the same stale catalog and reports success. Applying it needs a
  full quit and relaunch. **Offer it; never run it, and never edit
  `settings.json`.** `scripts/check-plugin-state.ps1` is the deeper version when
  the two local fields aren't enough.
  *(Lives here rather than in `/start` since 1.25.0: it is periodic and
  deliberate, which is the right cadence for it. Be warned that it is **not
  observed working here either** — a probe run on 2026-08-19 skipped it in this
  skill exactly as three runs skipped it in `/start`. Every check in this list
  that concerns the project gets run; this is the only one that leaves it, and it
  is the only one that gets dropped. If you are reading this because you want it
  to actually fire, the wording is not the problem — three rewrites at three
  positions changed nothing. Skill-to-skill invocation is the one mechanism
